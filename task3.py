...
# реализовать метод make_order(), принимающий экземпляр класса и количество ячеек в ряду. Данный
...
# то в последний ряд записываются все оставшиеся. Например, количество ячеек клетки равняется 12, количество
...
class Cell:
    ...
    def __sub__(self, other):
        residual = self.quantity - other.quantity
        if residual > 0:
            return residual
        else:
...
print(f'Сложение клеток: {cells1 + cells2}')
print(f'Операция вычитания: {cells2 - cells1}')
print(f'Операция умножения: {cells1 * cells2}')
print(f'Операция деления: {cells1 / cells2}')
print(f'Метод make_order() для первой клетки: {cells1.make_order(5)}')
print(f'Метод make_order() для второй клетки: {cells2.make_order(3)}')
